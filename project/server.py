...
total_packets = 10000000 # Total number of packets to be received, as per assignment condition
seq_num = 0 # Sequence number
packet_size = 4 # Packet size, a small packet size allows for faster iteration and testing of the networking code. It reduces the time required for transmitting and processing each packet, which can be beneficial when debugging or experimenting with different network configurations
max_seq_num = 65536 # Maximum sequence number,as per assignment condition 2^16
